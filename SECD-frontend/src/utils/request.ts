...
service.interceptors.response.use(
  (response: AxiosResponse<BaseResponse>) => {
    ...
    if (res.code && ![ResultEnum.SUCCESS, ResultEnum.CREATED].includes(res.code)) {
...
export async function request(_url: string | RequestOptions, _config: RequestOptions = {}) {
  ...
  try {
    ...
    const response = (await service.request({
      ...
      headers: {
        ...config.headers,
        ...(config.requestType === 'form' ? { 'Content-Type': 'multipart/form-data' } : {}),
      },
    ...
    if (data && Reflect.has(data, 'code')) {
      const hasSuccess = data.code === ResultEnum.SUCCESS || data.code === ResultEnum.CREATED;
      if (hasSuccess) {
    ...
    console.log('isReturnResult:', config.isReturnResult);
    // return data;
    return config.isReturnResult === false ? data : data.data;
  } catch (error: any) {
