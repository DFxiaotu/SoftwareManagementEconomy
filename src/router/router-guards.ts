...
export function createRouterGuards(router: Router, whiteNameList: WhiteNameList) {
  router.beforeEach(async (to, from, next) => {
    ...
    console.log('router-guard.ts', router.getRoutes());

    if (!token || userType == 'unknown') {
      // not login
      ...
      if (userStore.checkAuth()) {
        if (
          [LOGIN_NAME, REGISTER_NAME, AUTH_NAME].includes(String(to.name)) ||
          to.path === '/' ||
          (userType == 'doctor' && to.path.match('/patient*')) ||
          (userType == 'patient' && to.path.match('/doctor*'))
        ) {
          // 防止跳转到其它用户类型的页面
          next({ path: `/${userType}` });
        } else {
          ...
          if (userStore.menus.length === 0) {
            // 从后台获取菜单
            const [err] = await _to(userStore.fetchMenus());
            if (err) {
      ...
      } else if (to.name === AUTH_NAME) {
        next();
      } else {
        // 未认证的用户需要先进行认证
        next({ name: AUTH_NAME, query: { redirect: to.fullPath }, replace: true });
      }
    }
  });
